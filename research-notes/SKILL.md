...
- **Reproduce every named group the source sidebar has — even with a single captured child.** If the sidebar has a `Variables` group and you've captured only its `Use variables` page, still create `{ "Variables" = [ { "Use variables" = … } ] }`. Do **not** flatten that page up as a sibling of the neighboring groups — the reader navigates the notes nav against the live sidebar, so a flattened single entry breaks the 1:1 mapping and reads as misplaced ("bad nav"). Position the group where the sidebar puts it; fill in its other children as they're captured. (Verified failure 2026-07-13: `values/variables` was flattened next to `Resources` instead of nested under a `Variables` group — corrected on user report.)
- **The "stays flat" exception is only for levels *you* would invent, never for a level the source names.** If the source sidebar does not have a group there and you'd be adding a pure-taxonomy wrapper of your own, then a single child with no captured overview page stays flat — don't bury one note under an invented header. But if the source names the group, the rule above wins: reproduce it.
